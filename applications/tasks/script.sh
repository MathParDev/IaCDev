...
echo "docker run -d --network=$networkName --network-alias $networkAlias --name $containerName -e 'spring_profiles_active=$profiles' -e 'ENVIRONMENT=$environment' -e 'SECRETMANAGER_URL=$secretmanagerUrl' -p 9598:8080 $fullImage"

...
echo "docker run -d --network=$networkName --name $containerName -e 'spring_profiles_active=$profiles' -e 'ENVIRONMENT=$environment' -e 'SECRETMANAGER_URL=$secretmanagerUrl' -p 9595:8080 $fullImage"

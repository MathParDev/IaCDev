...
echo "docker run -d --network=$networkName -v $propertiesFolder:$propertiesFolder --network-alias $networkAlias --name $containerName -e 'JAVA_OPTS=-Dspring.profiles.active=$profiles' -e 'ENVIRONMENT=$environment' -e 'PROPERTIES_PREFIX=$propertiesFolder' $fullImage"

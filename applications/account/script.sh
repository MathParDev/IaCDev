...
echo "docker run -d --network=$networkName --network-alias $networkAlias --name $containerName -e 'ACTIVE_PROFILES=$profiles' -e 'ENVIRONMENT=$environment' -p 9596:8080 -e 'SECRETMANAGER_URL=$secretmanagerUrl' $fullImage"

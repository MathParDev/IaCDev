...
echo "docker run -d --network=$networkName --network-alias $networkAlias --name $containerName -e 'ACTIVE_PROFILES=$profiles' -e 'ENVIRONMENT=$environment' -e 'SECRETMANAGER_URL=$secretmanagerUrl' $fullImage"
